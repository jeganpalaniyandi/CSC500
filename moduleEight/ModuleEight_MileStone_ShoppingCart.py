...
class ShoppingCart:
    ...
    def remove_item(self):
        """
            Prompt the user to enter the item name that needs to be removed.
            If the entered item name is in the cart, remove it, else
            display, 'Item not found in cart. Nothing removed.'
        """
        item_to_remove = input("Enter the item name to be removed from cart --> ")

        for item in self.cart_items:
            if item.item_name == item_to_remove:
                self.cart_items.remove(item)
                print('{} removed from the cart.'.format(item_to_remove))
                return
    ...
    def modify_item(self):
        ...
        """
        item_to_modify = input("Enter the item name to be modified --> ")
        new_quantity = int(input("Enter the new quantity --> "))

        for item in self.cart_items:
            if item.item_name == item_to_modify:
                item.item_quantity = new_quantity
                print('{} quantity is updated as {}.'.format(item_to_modify, new_quantity))
                return
